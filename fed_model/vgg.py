...
class VGG(nn.Module):
    def __init__(self, nn_arch, dataset, use_bn=True):
        ...
        inter_feature = 2048 if self.dataset == "dsprint" else 512
        
        self.intermediate_classifier = nn.Sequential(
            nn.Dropout(),
            nn.Linear(inter_feature, 512),
            nn.ReLU(True),
            nn.Dropout(),
            nn.Linear(512, 512),
            nn.ReLU(True),
    ...
    def forward_test(self, x):
        layers_list = list(self.features)
        output = []
        for s_layer in layers_list:
            x = s_layer(x)
